import { Fragment, useTransition, useRef, useMemo } from "react";
import { useParams } from "next/navigation";
import { Dialog, Transition } from "@headlessui/react";
import { AlertTriangleIcon } from "lucide-react";
import { getStoryLinkDisplay } from "@/lib/editor";
import { deleteItem } from "@/app/edit/[date]/actions";
import type { StoryLink } from "@/types/stories";

type Props = {
    open: boolean;
    setOpen: (newState: boolean) => void;
    item: StoryLink;
    index: number;
    groupIndex?: number;
};

const DeleteItem = ({ open, setOpen, item, index, groupIndex }: Props) => {
    const params = useParams() as { date: string };
    const [_, startTransition] = useTransition();

    const type = useMemo(
        () =>
            typeof item === "string" &&
            (item.includes("stories)") || item.includes("story)"))
                ? "Group"
                : "Link",
        [item],
    );
    const cancelButtonRef = useRef(null);

    const deleteItemWrapper = () => {
        startTransition(() => deleteItem(params.date, index, true, groupIndex));
        setOpen(false);
    };

    return (
        <Transition.Root show={open} as={Fragment}>
            <Dialog
                as="div"
                className="relative z-10"
                initialFocus={cancelButtonRef}
                onClose={setOpen}
            >
                <Transition.Child
                    as={Fragment}
                    enter="ease-out duration-300"
                    enterFrom="opacity-0"
                    enterTo="opacity-100"
                    leave="ease-in duration-200"
                    leaveFrom="opacity-100"
                    leaveTo="opacity-0"
                >
                    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" />
                </Transition.Child>

                <div className="fixed inset-0 z-10 overflow-y-auto">
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <Transition.Child
                            as={Fragment}
                            enter="ease-out duration-300"
                            enterFrom="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                            enterTo="opacity-100 translate-y-0 sm:scale-100"
                            leave="ease-in duration-200"
                            leaveFrom="opacity-100 translate-y-0 sm:scale-100"
                            leaveTo="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                        >
                            <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all dark:bg-hk-grey sm:my-8 sm:w-full sm:max-w-lg">
                                <div className="px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                    <div className="sm:flex sm:items-start">
                                        <div className="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
                                            <AlertTriangleIcon
                                                className="h-6 w-6 text-red-600"
                                                aria-hidden="true"
                                            />
                                        </div>
                                        <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                            <Dialog.Title
                                                as="h3"
                                                className="text-base font-semibold leading-6 text-gray-900 dark:text-gray-100"
                                            >
                                                Delete {type}
                                            </Dialog.Title>
                                            <div className="mt-1 space-y-2">
                                                <p className="text-sm text-gray-500">
                                                    Are you sure you want to
                                                    delete the following{" "}
                                                    {type.toLowerCase()}? This
                                                    action is irreversible.
                                                </p>
                                                <p className="border-l-2 border-gray-500 py-1 pl-3 text-sm text-gray-500">
                                                    {type === "Group"
                                                        ? (item as string)
                                                        : getStoryLinkDisplay(
                                                              item,
                                                          )}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                    <button
                                        type="button"
                                        className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm transition-colors duration-150 ease-in-out hover:bg-red-500 sm:ml-3 sm:w-auto"
                                        onClick={() => deleteItemWrapper()}
                                    >
                                        Delete
                                    </button>
                                    <button
                                        type="button"
                                        className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-medium text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 transition-colors duration-150 ease-in-out hover:bg-gray-50 dark:bg-hk-grey dark:text-gray-100 dark:ring-gray-700 dark:hover:bg-hk-grey-hover sm:mt-0 sm:w-auto"
                                        onClick={() => setOpen(false)}
                                        ref={cancelButtonRef}
                                    >
                                        Cancel
                                    </button>
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
                    </div>
                </div>
            </Dialog>
        </Transition.Root>
    );
};

export default DeleteItem;
